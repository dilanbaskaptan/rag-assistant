...
```
  ...
  docs/*.md ──────► │  ingest.py  │ ── splits into chunks, embeds each one
                  ...
                  └────────┬─────────┘
                           │
   question ──────► retrieval.py ──── cosine similarity search (get_top_chunks)
                           │
                           ▼
...
├── retrieval.py       # Embeddings + cosine similarity + get_top_chunks()
...
- **The first question after starting the app is much slower than the rest (~50s vs a few seconds).** That's a one-time cost per Foundry Local restart — the GPU/TensorRT execution provider builds and optimizes its inference engine on the first real call. Every question after that in the same session is fast, though the exact time (roughly 2-8s observed) varies with whatever else is running on the machine. If the GPU split above isn't usable on a given machine (e.g. a GPU with less VRAM), falling back to CPU for both models makes every answer ~25-40s instead, with no first-call spike.
